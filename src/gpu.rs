...
pub fn matmul_gpu(a: &[f32], b: &[f32], m: usize, n: usize, k: usize) -> Result<Vec<f32>, String> {
    if a.len() != m * k {
        return Err(format!("A 需 m×k (got {} need {})", a.len(), m * k));
    }
    if b.len() != k * n {
        return Err(format!("B 需 k×n (got {} need {})", b.len(), k * n));
    }
    let spv = loc::read_kernel("matmul_var").map_err(|e| format!("spv: {e}"))?;
    ...
    ses.dispatch(&[ab, bb, pb], groups, &mut out)
