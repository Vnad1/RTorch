...
fn find_engine() -> std::io::Result<*mut c_void> {
    ...
    Err(std::io::Error::other(
        "rtorch_vk.dll not found (build vk_engine.cpp) — see vk_engine.cpp header",
    ))
}
...
impl VkSession {
    ...
    pub fn init(spv: &[u8], input_sizes: &[usize], out_len: usize) -> std::io::Result<VkSession> {
        ensure_path();
        ...
        let (init, dispatch, destroy) = match (init, dispatch, destroy) {
            ...
            _ => {
                ...
                return Err(std::io::Error::other(
                    "engine symbols not exported (old DLL?)",
                ));
            }
        };
        let engine = Engine {
    ...
    pub fn dispatch(
        ...
        let reuse = if self.last_inputs.as_deref() == Some(inputs) {
            1
        } else {
            0
        };
        ...
        let in_ptrs: Vec<*const c_void> =
            inputs.iter().map(|b| b.as_ptr() as *const c_void).collect();
        ...
        if rc != 0 {
            return Err(std::io::Error::other(format!(
                "rtorch_vk_dispatch failed rc={rc}"
            )));
        }
        Ok(elapsed)
    }
...
impl Drop for VkSession {
    fn drop(&mut self) {
        unsafe {
            (self.engine.destroy)(self.ctx);
        }
        unsafe {
            FreeLibrary(self.engine.handle);
        }
    }
}
...
type DevPipeAddFn =
    unsafe extern "C" fn(i32, *const c_void, usize, *const i32, i32, i32, u32, u32, u32) -> i32;
...
impl GpuDevice {
    ...
    pub fn new() -> std::io::Result<GpuDevice> {
        ...
        let (
            ...
            (
                ...
                Some(k2),
                Some(l2),
            ) => (a, b, c, d, e, g, h, f2, i, j, k2, l2),
            _ => {
                unsafe { FreeLibrary(handle) };
                return Err(std::io::Error::other(
                    "device-resident symbols not exported (rebuild vk_engine.cpp)",
                ));
            }
        };
        let dev = unsafe { init() };
        if dev < 0 {
            unsafe { FreeLibrary(handle) };
            return Err(std::io::Error::other(format!(
                "rtorch_vk_dev_init failed (dev={dev})"
            )));
        }
        Ok(GpuDevice {
            handle,
    ...
    pub fn pipe_bind(&self, pipe: i32, in_bufs: &[i32], out_buf: i32) -> std::io::Result<()> {
        ...
        if rc != 0 {
            return Err(std::io::Error::other(format!(
                "rtorch_vk_pipe_bind failed rc={rc}"
            )));
        }
        Ok(())
    }
    ...
    pub fn pipe_run(&self, pipe: i32, groups: [u32; 3]) -> std::io::Result<()> {
        ...
        if rc != 0 {
            return Err(std::io::Error::other(format!(
                "rtorch_vk_pipe_run failed rc={rc}"
            )));
        }
        Ok(())
    }
    ...
    pub fn dev_begin(&self) -> std::io::Result<()> {
        ...
        if rc != 0 {
            return Err(std::io::Error::other(format!(
                "rtorch_vk_dev_begin failed rc={rc}"
            )));
        }
        Ok(())
    }
    ...
    pub fn dev_pipe_record(&self, pipe: i32, groups: [u32; 3]) -> std::io::Result<()> {
        ...
        if rc != 0 {
            return Err(std::io::Error::other(format!(
                "rtorch_vk_pipe_record failed rc={rc}"
            )));
        }
        Ok(())
    }
    ...
    pub fn dev_submit(&self, wait: bool) -> std::io::Result<()> {
        ...
        if rc != 0 {
            return Err(std::io::Error::other(format!(
                "rtorch_vk_dev_submit failed rc={rc}"
            )));
        }
        Ok(())
    }
