...
fn check_agreement(m: usize, k: usize, n: usize) -> Result<(), String> {
    ...
    if gpu.len() != m * n { return Err(format!("len mismatch gpu={} expected {}", gpu.len(), m * n)); }
    for i in 0..m * n {
        let g = gpu[i];
        let c = cpu[i];
        if g.is_nan() || g.is_infinite() { return Err(format!("GPU NaN/Inf at [{i}]: gpu={g} cpu={c} (m={m} k={k} n={n})")); }
        if c.is_nan() || c.is_infinite() { return Err(format!("CPU NaN/Inf at [{i}]")); }
        let abs = (g - c).abs();
        let rel = abs / c.abs().max(1e-6);
        if abs > 1e-1 && rel > 1e-2 {
            return Err(format!("mismatch[{i}] gpu={g} cpu={c} (m={m} k={k} n={n} abs={abs} rel={rel})"));
        }
    }
    Ok(())
}
...
fn gpu_matches_cpu_non_aligned_shapes() {
    ...
    for &(m, k, n) in &shapes {
        if let Err(e) = check_agreement(m, k, n) {
            ...
            if e.contains("gpu err") { eprintln!("GPU unavailable (skip): {e}"); continue; }
            panic!("{e}");
        }
        checked += 1;
    }
...
fn gpu_matmul_len_and_shape_are_correct() {
    ...
    match matmul_gpu(&a, &b, m, n, k) {
        Ok(c) => { assert_eq!(c.len(), m * n, "output length must be m*n"); }
        Err(e) => { eprintln!("GPU unavailable (skip): {e}"); }
    }
}
...
fn lcg(seed: &mut u64) -> u64 { // deterministic LCG for reproducible fuzz
...
fn fuzz_cpu_ops_never_silent_wrong() {
    ...
    for _ in 0..200 {
        ...
        let a = Tensor::from_data((0..r * c).map(|i| (lcg(&mut seed) % 100) as f64 / 7.0).collect(), r, c);
        let c2 = (lcg(&mut seed) % 6 + 1) as usize;
        let b = Tensor::from_data((0..c * c2).map(|i| (lcg(&mut seed) % 100) as f64 / 11.0).collect(), c, c2);
        // matmul must match reference
        let out = tensor::matmul(&a, &b);
        for i in 0..r { for j in 0..c2 {
            let mut s = 0.0;
            for k in 0..c { s += a.data[i * c + k] * b.data[k * c2 + j]; }
            assert!((out.data[i * c2 + j] - s).abs() < 1e-9, "fuzz matmul mismatch");
        }}
        // tanh matches std
        let t = tensor::tanh(&a);
        for i in 0..a.len() { assert!((t.data[i] - a.data[i].tanh()).abs() < 1e-12); }
        // softmax rows sum to 1
        let s = tensor::softmax_row(&a);
        for row in 0..r { let sum: f64 = (0..c).map(|j| s.data[row * c + j]).sum(); assert!((sum - 1.0).abs() < 1e-9); }
    }
}
