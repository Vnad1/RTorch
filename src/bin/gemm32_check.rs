...
fn main() {
    ...
    for (r, k, c) in [
        ...
        let e16 = max_err(&m16, &refm);
        let e32 = max_err(&m32, &refm);
        println!(
            "{r}×{k}×{c}: tile16 err={e16:.2e}  tile32 err={e32:.2e}  {}",
            if e32 < 1e-2 { "PASS" } else { "FAIL" }
        );
    }
    ...
    for n in [512usize, 1024usize] {
        ...
        let _ = gpu_tensor::matmul32(&ta, &tb);
        // tile16
        let reps = 30;
        ctx.begin_batch();
        for _ in 0..reps {
        ...
        println!(
            "n={n}: tile16={:.3}ms {:.0}GF  tile32={:.3}ms {:.0}GF  提速 {:.2}×",
            dt16 * 1e3,
            fl / (dt16 * 1e9),
            dt32 * 1e3,
            fl / (dt32 * 1e9),
