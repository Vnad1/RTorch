...
use rtorch::autograd::{add, backward, leaf, matmul, softmax_row, tanh, Adam, Var};
use std::rc::Rc;

fn make_net(seed: u64) -> (Var, Var, Var, Var, Adam) {
    let mut st = seed;
    let mut rng = move || { st = st.wrapping_add(0x9E3779B97F4A7C15); let mut z = st;
    ...
    (leaf(rv(6*6), 6, 6), leaf(rv(6*4), 6, 4), leaf(rv(3*6), 3, 6), leaf(vec![0.0;6], 6, 1), Adam::new(0.05))
}
fn params(w: &[Var]) -> Vec<Var> { w.to_vec() }

fn train_step(w: &[Var], opt: &mut Adam, x: &[f64], y: &[usize], lr: f64) -> f64 {
    let d = 6usize; let o = 4usize; let cl = 3usize;
    ...
    let mut dl = pr.clone(); dl[tg] -= 1.0;
    logits.borrow_mut().grad = dl;
    backward(&logits);
    opt.lr = lr;
    let ps = params(w);
    opt.step(&ps);
    for p in &ps { p.borrow_mut().grad.fill(0.0); }
    loss
}
...
fn main() {
    ...
    // 固定输入/目标序列(确定性)
    let xs: Vec<Vec<f64>> = (0..8).map(|i| (0..6).map(|j| (((i*3+j)%13) as f64)/6.0).collect()).collect();
    let ys: Vec<usize> = (0..8).map(|i| (i*2)%3).collect();

    // A) 不中断 8 步
    let (w1, w2, w3, w4, mut optA) = make_net(0x1234);
    let netA = vec![w1, w2, w3, w4];
    for i in 0..8 { train_step(&netA, &mut optA, &xs[i], &[ys[i]], 0.05); }
    ...
    for i in 0..4 { train_step(&netB, &mut optB, &xs[i], &[ys[i]], 0.05); }
    ...
    for i in 4..8 { train_step(&netB, &mut optB, &xs[i], &[ys[i]], 0.05); }
    let finalB: Vec<f64> = netB[0].borrow().data.clone();

    let maxe = finalA.iter().zip(&finalB).map(|(a,b)| (a-b).abs()).fold(0.0f64, f64::max);
    println!("连续8步 vs 中断(4+4)权重 max|Δ| = {maxe:.3e}  => {}", if maxe < 1e-12 { "PASS" } else { "FAIL" });
    ...
    for i in 0..4 { train_step(&netC0, &mut optC0, &xs[i], &[ys[i]], 0.05); }
    ...
    for i in 4..8 { train_step(&netC0, &mut optC, &xs[i], &[ys[i]], 0.05); }
    let finalC: Vec<f64> = netC0[0].borrow().data.clone();
    let maxe2 = finalA.iter().zip(&finalC).map(|(a,b)| (a-b).abs()).fold(0.0f64, f64::max);
    println!("load_state 恢复续训 vs 不中断 max|Δ| = {maxe2:.3e}  => {}", if maxe2 < 1e-9 { "PASS" } else { "FAIL" });
