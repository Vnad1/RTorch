...
pub fn bcast_shape(r1: usize, c1: usize, r2: usize, c2: usize) -> Result<(usize, usize), String> {
    let r = if r1 == r2 {
        ...
        return Err(format!(
            "shape: cannot broadcast {r1}×{c1} and {r2}×{c2} (row dims {r1} vs {r2} not equal or 1)"
        ));
    };
    let c = if c1 == c2 {
        c1
        ...
        return Err(format!(
            "shape: cannot broadcast {r1}×{c1} and {r2}×{c2} (col dims {c1} vs {c2} not equal or 1)"
        ));
    };
    Ok((r, c))
}
...
impl Tensor {
    pub fn zeros(r: usize, c: usize) -> Self {
        mk(vec![0.0; r * c], vec![r, c], DType::F64)
    }
    pub fn ones(r: usize, c: usize) -> Self {
        mk(vec![1.0; r * c], vec![r, c], DType::F64)
    }
    pub fn from_data(data: Vec<f64>, r: usize, c: usize) -> Self {
        mk(data, vec![r, c], DType::F64)
    }
    ...
    pub fn reshape(&self, dims: Vec<usize>) -> Result<Tensor, String> {
        ...
        if numel != self.numel() {
            return Err(format!(
                "shape: reshape numel mismatch get {} want {} for {:?}",
                self.numel(),
                numel,
                dims
            ));
    ...
    pub fn view(&self, dims: Vec<usize>) -> Result<Tensor, String> {
        self.reshape(dims)
    }
...
pub fn add(a: &Tensor, b: &Tensor) -> Tensor {
    ...
    let mut out = Tensor::zeros(r, c);
    out.dtype = if a.dtype == b.dtype {
        a.dtype
    } else {
        panic!(
            "dtype: add dtype mismatch {} vs {}",
            a.dtype.name(),
            b.dtype.name()
        )
    };
...
pub fn softmax_row(x: &Tensor) -> Tensor {
    ...
    for i in 0..x.r {
        let start = i * x.c;
        let m = x.data[start..start + x.c]
            .iter()
            .cloned()
            .fold(f64::NEG_INFINITY, f64::max);
        let mut esum = 0.0;
        for j in 0..x.c {
            let e = (x.data[start + j] - m).exp();
            out.data[start + j] = e;
            esum += e;
        }
        for j in 0..x.c {
            out.data[start + j] /= esum;
        }
    }
    out
}
