...
fn main() {
    ...
    {
        let n = 16usize;
        let a = vec![1.0f32; n * n]; let b = vec![1.0f32; n * n];
        ...
        for _ in 0..reps { let m = gpu_tensor::matmul(&ta, &tb); std::hint::black_box(&m); }
        let dt = t0.elapsed().as_secs_f64() / reps as f64;
        println!("① 极小微 matmul n={n} x{reps}: {:.3} ms/op  (≈ 单 op 固定开销: sync+submit+重绑)", dt * 1e3);
    }
    ...
    for n in [256usize, 512usize, 1024usize] {
        ...
        println!("② matmul n={n}: {:.3} ms/op -> {:.1} GFLOPS (含sync)", dt * 1e3, flops / (dt * 1e9));
    }
    ...
    // ③ 链 matmul→add→tanh (3 op, 3 sync) vs 单独 matmul 1 op: 看链的额外开销.
    {
        ...
        let _ = { let m = gpu_tensor::matmul(&ta, &tb); let x = gpu_tensor::add(&m, &tbias); gpu_tensor::tanh(&x); };
        let t0 = std::time::Instant::now();
        for _ in 0..reps { let m = gpu_tensor::matmul(&ta, &tb); let x = gpu_tensor::add(&m, &tbias); let y = gpu_tensor::tanh(&x); std::hint::black_box(&y); }
        let dt3 = t0.elapsed().as_secs_f64() / reps as f64;
        println!("③ 链 matmul→add→tanh n={n}: {:.3} ms/op (3 op)  → 平均 {:.3} ms/op  vs 单op {:.3} ms", dt3 * 1e3, dt3 * 1e3 / 3.0, dt1 * 1e3);
    }

    // ④ to_vec 下载成本 (B×V, softmax/loss 路径): B=128 V=3401 (train25m 配置)
    {
        let b = 128usize; let v = 3401usize;
        ...
        let reps = 30;
        let t0 = std::time::Instant::now();
        for _ in 0..reps { let h = t.to_vec(); std::hint::black_box(h); }
        let dt = t0.elapsed().as_secs_f64() / reps as f64;
        let mb = (b * v * 4) as f64 / 1e6;
        println!("④ to_vec 下载 B={b} V={v} ({mb:.0} MB): {:.3} ms/次  {:.1} GB/s", dt * 1e3, mb / dt / 1000.0);
    }

    // ⑤ add 广播成本 (host 往返: to_vec→host tile→重传): bias 1×n → B×n
    {
        let b = 128usize; let n = 512usize;
        ...
        let reps = 30;
        let t0 = std::time::Instant::now();
        for _ in 0..reps { let x = gpu_tensor::add(&ta, &tb); std::hint::black_box(&x); }
        let dt = t0.elapsed().as_secs_f64() / reps as f64;
        println!("⑤ add 广播 bias(1×{n})→B={b}: {dt:.3} ms/次  (含 host tile+重传往返: 若有大量add广播是痛点)");
    }
    ...
    // ⑦ softmax GPU (row-wise, B×V) correctness + perf vs CPU
    {
        ...
        for r in 0..b {
            ...
            for c0 in 0..v { let refv = ex[c0] / ssum; maxe = maxe.max((sh[r * v + c0] - refv).abs()); }
        }
        println!("⑦ softmax GPU B={b} V={v}: max_err={maxe:.3e} {}", if maxe < 1e-3 { "PASS" } else { "FAIL" });
        let _ = gpu_tensor::softmax(&t);
        let reps = 30; let t0 = std::time::Instant::now();
        for _ in 0..reps { let s = gpu_tensor::softmax(&t); std::hint::black_box(&s); }
        let dt = t0.elapsed().as_secs_f64() / reps as f64;
        println!("① softmax GPU 时间: {:.3} ms/次 (vs to_vec 下载 5.6 ms → GPU softmax 免下载)", dt * 1e3);
    }
    ...
    // ⑥ CPU 对照 matmul (同规模, 参考)
    {
        ...
        for _ in 0..reps {
            ...
            for i in 0..n { for j in 0..n { let mut acc = 0.0; for k in 0..n { acc += a[i * n + k] * b[k * n + j]; } out[i * n + j] = acc; } }
            std::hint::black_box(&out);
        }
        let dt = t0.elapsed().as_secs_f64() / reps as f64;
        println!("⑥ CPU matmul n={n}: {:.1} ms/op -> {:.1} GFLOPS (对照)", dt * 1e3, 2.0 * (n as f64).powi(3) / (dt * 1e9));
    }
