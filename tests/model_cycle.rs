...
fn step(x: &autograd::Var, target: &[f64], w: &autograd::Var, b: &autograd::Var, opt: &mut Adam) -> f64 {
...
fn train_save_load_continue_full_cycle() {
    ...
    for _ in 0..300 { step(&x, &target, &w, &b, &mut opt); }
    let before = mse(&{
        let pred = autograd::add(&autograd::matmul(&x, &w), &b);
    ...
    let f32s = |v: &Vec<Vec<f64>>| v.iter().map(|l| l.iter().map(|&x| x as f32).collect::<Vec<f32>>()).collect::<Vec<Vec<f32>>>();
    let model = rtw::Model {
        name: "demo-net".into(),
        version: 1,
        params: vec![
            rtw::NamedTensor { name: "W".into(), shape: vec![1, 1], dtype: rtw::DTYPE_FP32, data: w.borrow().data.iter().map(|&f| f as f32).collect() },
            rtw::NamedTensor { name: "b".into(), shape: vec![1, 1], dtype: rtw::DTYPE_FP32, data: b.borrow().data.iter().map(|&f| f as f32).collect() },
        ],
        opt: Some(rtw::OptState { m: f32s(&ms), v: f32s(&vs), t }),
    };
    ...
    let w2 = autograd::from_data(restored.params[0].data.iter().map(|&f| f as f64).collect(), 1, 1);
    let b2 = autograd::from_data(restored.params[1].data.iter().map(|&f| f as f64).collect(), 1, 1);
    let mut opt2 = Adam::new(0.05);
    if let Some(o) = restored.opt {
        let tof = |v: &Vec<Vec<f32>>| v.iter().map(|l| l.iter().map(|&x| x as f64).collect::<Vec<f64>>()).collect::<Vec<Vec<f64>>>();
        opt2.load_state(&[w2.clone(), b2.clone()], &tof(&o.m), &tof(&o.v), o.t);
    }
    let mut last = mse(&{ let pred = autograd::add(&autograd::matmul(&x, &w2), &b2); pred.borrow().data.clone() }, &target);
    for _ in 0..200 { last = step(&x, &target, &w2, &b2, &mut opt2); }

    assert!(last < before.max(1.0), "reload + continue not improving: {before} -> {last}");
    assert!(last < 1e-2, "model did not converge after checkpoint reload: mse {last}");
    // w approaches ~2 (we fit 2x).
    assert!((w2.borrow().data[0] - 2.0).abs() < 0.2, "W={} expected ~2", w2.borrow().data[0]);
}
