...
fn matmul_k_mismatch_panics() {
    ...
    assert!(
        res.is_err(),
        "matmul with a.c != b.r must error, not silently compute"
    );
}
...
fn add_incompatible_shapes_panic() {
    ...
    assert!(
        res.is_err(),
        "add 2×2 with 3×2 must error (was silently wrapping)"
    );
}
...
fn gather_out_of_range_panics() {
    ...
    assert!(
        res.is_err(),
        "gather index 5 on size-2 embedding must error (was clamping)"
    );
}
