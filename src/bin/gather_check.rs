...
fn main() {
    ...
    for (i, &id) in ids.iter().enumerate() {
        if id >= ve { continue; }
        for j in 0..e { refgrad[id * e + j] += dc[i * e + j]; }
    }
    let mut maxe = 0.0f64;
    for i in 0..ve * e { let d = (gemb_grad[i] - refgrad[i]).abs(); if d > maxe { maxe = d; } }
    println!("emb.grad max|GPU-CPU| = {maxe:.3e}  => {}", if maxe < 1e-5 { "PASS" } else { "FAIL" });
    // 只在 ids 对应行非零
    let mut crow = 0usize; for (i,&id) in ids.iter().enumerate() { if id==3 { crow = i; break; } }
    println!("row3 grad[..4]: GPU={:?} CPU={:?}", &gemb_grad[3*e..3*e+4], &refgrad[3*e..3*e+4]);
    println!("\n[gather-check] DONE");
}
