...
impl Net {
    fn new(ctx: Rc<GpuContext>, d: usize, e: usize, v: usize) -> Self {
        ...
        Net {
            ...
            e,
            v,
            a: gvar::leaf(Rc::clone(&ctx), rv(d * d), d, d),
            emb: gvar::leaf(Rc::clone(&ctx), rv(v * e), v, e),
            bm: gvar::leaf(Rc::clone(&ctx), rv(e * d), e, d),
            bias: gvar::leaf(Rc::clone(&ctx), vec![0.0; d], 1, d),
            c: gvar::leaf(Rc::clone(&ctx), rv(d * v), d, v),
            opt: AdamG::new(0.001),
        }
    }
    fn params(&self) -> Vec<GVar> {
        vec![
            Rc::clone(&self.a),
            Rc::clone(&self.emb),
            Rc::clone(&self.bm),
...
fn sum_loss(loss_ts: &[GpuTensor]) -> f64 {
    loss_ts.iter().map(|t| t.to_vec()[0] as f64).sum::<f64>()
}
...
fn main() {
    ...
    for _ in 0..10 {
        let xs = netA.train_batch(&toks, &target, b, l);
        lossesA.push(sum_loss(&xs) / (b * l) as f64);
    }
    ...
    println!(
        "耗时(10步): 逐op={tA:.3}s  batch={tB:.3}s  提速 {:.2}×",
        tA / tB
    );
    let a_now = gvar::to_vec(&netA.a);
    let b_now = gvar::to_vec(&netB.a);
    let wmax = a_now
        .iter()
        .zip(&b_now)
        .map(|(x, y)| (x - y).abs())
