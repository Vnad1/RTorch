...
fn spv(name: &str) -> std::io::Result<Vec<u8>> {
    loc::read_kernel(name)
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::NotFound, format!("spv {name}: {e}")))
}
...
impl KernelRegistry {
    pub fn insert(&mut self, name: &str, pipe: i32) {
        self.map.insert(name.to_string(), pipe);
    }
...
impl GpuContext {
    ...
    pub fn new() -> std::io::Result<GpuContext> {
        ...
        kernels.insert(
            "gemm_tiled",
            pipe_kernel(&dev, "gemm_tiled", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "gemm_tile32",
            pipe_kernel(&dev, "gemm_tile32", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "add",
            pipe_kernel(&dev, "add", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "tanh",
            pipe_kernel(&dev, "tanh", &[scratch, s2], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "scale",
            pipe_kernel(&dev, "scale", &[scratch, s2], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "mmgrad_a",
            pipe_kernel(&dev, "mmgrad_a", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "mmgrad_b",
            pipe_kernel(&dev, "mmgrad_b", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        kernels.insert(
            "tanhgrad",
            pipe_kernel(&dev, "tanhgrad", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        // gather: bind 0=E,1=ids,2=params,3=out
        kernels.insert(
            "gather",
            pipe_kernel(&dev, "gather", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        // softmax-CE: bind 0=logits,1=target,2=params,3=grad,4=loss
        kernels.insert(
            "ce",
            pipe_kernel(&dev, "ce", &[scratch, s2, s3, scratch], scratch, [1, 1, 1])?,
        );
        // row softmax: bind 0=A(B×V),1=P(B,V),2=C(B×V)
        kernels.insert(
            "softmax",
            pipe_kernel(&dev, "softmax", &[scratch, s2, s3], scratch, [1, 1, 1])?,
        );
        ...
        kernels.insert(
            "reduce",
            pipe_kernel(&dev, "reduce", &[scratch, s2], scratch, [1, 1, 1])?,
        );
        Ok(GpuContext {
            dev,
            kernels,
            batch: std::cell::Cell::new(false),
            free_pending: std::cell::RefCell::new(Vec::new()),
            params_cache: std::cell::RefCell::new(std::collections::HashMap::new()),
        })
    }
    ...
    pub fn pipe(&self, name: &str) -> i32 {
        self.kernels
            .get(name)
            .unwrap_or_else(|| panic!("kernel not registered: {name}"))
    }

    fn alloc_out(&self, n: usize) -> i32 {
        self.dev.alloc(n * 4)
    }
...
pub fn matmul(a: &GpuTensor, b: &GpuTensor) -> GpuTensor {
    ...
    a.ctx.run_op(
        a.ctx.pipe("gemm_tile32"),
        &inb,
        out.buf,
        [((c as u32) + 31) / 32, ((r as u32) + 31) / 32, 1],
    );
    out
}

// matmul 变体: 等价 tile32 (16 兼容已并入 matmul). 保留供对拍/回溯.
pub fn matmul32(a: &GpuTensor, b: &GpuTensor) -> GpuTensor {
    matmul(a, b)
}
...
pub fn gather(emb: &GpuTensor, ids: &[usize], b: usize) -> GpuTensor {
    ...
    ctx.run_op(
        ...
        &inb,
        out.buf,
        [((b * e) as u32 + 255) / 256, 1, 1],
    );
    ctx.ctx_free(ib);
    out
}

fn bytes_from_u32(v: &[u32]) -> Vec<u8> {
    v.iter().flat_map(|x| x.to_le_bytes()).collect()
}
...
pub fn f64_to_f32(d: &[f64]) -> Vec<f32> {
    d.iter().map(|&x| x as f32).collect()
}
pub fn f32_to_f64(d: &[f32]) -> Vec<f64> {
...
pub fn matmul_backward(a: &GpuTensor, b: &GpuTensor, dC: &GpuTensor) -> (GpuTensor, GpuTensor) {
    ...
    a.ctx.run_op(
        ...
        &ina,
        da.buf,
        [((m as u32) + 15) / 16, ((k as u32) + 15) / 16, 1],
    );
    // dB
    ...
    a.ctx.run_op(
        ...
        &inb,
        db.buf,
        [((k as u32) + 15) / 16, ((n as u32) + 15) / 16, 1],
    );
    (da, db)
}
