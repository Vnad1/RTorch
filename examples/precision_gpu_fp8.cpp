...
// fp8(E4M3) in-shader (simulated: no native GLSL fp8 type), accumulated fp32.
#include "rtorch.h"
#include <cstddef>
...
float q8(float x) {
    ...
    float mi = round(frac * 8.0);
    if (mi >= 8.0) { mi = 0.0; e += 1; }
    float q = exp2(float(e)) * (1.0 + mi / 8.0);
    return neg ? -q : q;
}
void main() {
    ...
    for (uint k = 0u; k < 64u; k++)
        acc += q8(float(a[row * 64u + k])) * q8(float(b[k * 64u + col]));
    o[i] = acc;
}
)GL";
}
