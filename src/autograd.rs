...
pub fn leaf(data: Vec<f64>, r: usize, c: usize) -> Var {
    Rc::new(RefCell::new(VarData { grad: vec![0.0; data.len()], data, r, c, parent: None }))
}
pub fn zeros(r: usize, c: usize) -> Var { leaf(vec![0.0; r * c], r, c) }
pub fn from_data(d: Vec<f64>, r: usize, c: usize) -> Var { leaf(d, r, c) }
fn wrap(d: Vec<f64>, r: usize, c: usize, p: Node) -> Var {
    Rc::new(RefCell::new(VarData { grad: vec![0.0; d.len()], data: d, r, c, parent: Some(Rc::new(p)) }))
}
pub fn len(v: &Var) -> usize { v.borrow().data.len() }
...
pub fn matmul(a: &Var, b: &Var) -> Var {
    ...
    for i in 0..ar { for j in 0..bc {
        let mut acc = 0.0;
        for k in 0..ac { acc += a.borrow().data[i * ac + k] * b.borrow().data[k * bc + j]; }
        out[i * bc + j] = acc;
    }}
...
pub fn backward(out: &Var) {
    ...
    fn dfs(v: &Var, order: &mut Vec<Var>, visited: &mut std::collections::HashSet<usize>) {
        ...
        if let Some(p) = v.borrow().parent.clone() {
            match &*p {
                Node::MatMul(a, b, ..) | Node::Add(a, b, ..) | Node::Mul(a, b, ..) => { dfs(a, order, visited); dfs(b, order, visited); }
                Node::Scale(a, ..) | Node::Tanh(a, ..) | Node::Sigmoid(a, ..) | Node::Gather(a, ..) => { dfs(a, order, visited); }
                Node::Stack(rows, ..) => { for r in rows { dfs(r, order, visited); } }
            }
        }
    ...
    for v in order.iter().rev() {
        let (outg, parent) = { let b = v.borrow(); (b.grad.clone(), b.parent.clone()) };
        ...
        match &*p {
            Node::MatMul(a, b, r, k, c) => {
                ...
                for i in 0..*r { for t in 0..*k {
                    let mut s = 0.0; for j in 0..*c { s += outg[i * *c + j] * b.borrow().data[t * *c + j]; }
                    a.borrow_mut().grad[i * *k + t] += s;
                }}
                for t in 0..*k { for j in 0..*c {
                    let mut s = 0.0; for i in 0..*r { s += a.borrow().data[i * *k + t] * outg[i * *c + j]; }
            ...
            Node::Scale(a, s, r, cc) => {
                for i in 0..*r { for j in 0..*cc { a.borrow_mut().grad[i * *cc + j] += s * outg[i * *cc + j]; } }
            }
            Node::Tanh(a, r, c) => {
            ...
            Node::Gather(e, i, c) => {
                for j in 0..*c { e.borrow_mut().grad[*i * *c + j] += outg[j]; }
            }
            Node::Stack(rows, b, c) => {
                for i in 0..*b { for j in 0..*c { rows[i].borrow_mut().grad[j] += outg[i * *c + j]; } }
            }
        }
    }
}
...
pub fn zero_grad(out: &Var) {
    ...
    fn dfs(v: &Var, visited: &mut std::collections::HashSet<usize>) {
        let id = Rc::as_ptr(v) as usize;
        if !visited.insert(id) { return; }
        for g in &mut v.borrow_mut().grad { *g = 0.0; }
        if let Some(p) = v.borrow().parent.clone() {
            match &*p {
                Node::MatMul(a, b, ..) | Node::Add(a, b, ..) | Node::Mul(a, b, ..) => { dfs(a, visited); dfs(b, visited); }
                Node::Scale(a, ..) | Node::Tanh(a, ..) | Node::Sigmoid(a, ..) | Node::Gather(a, ..) => { dfs(a, visited); }
                Node::Stack(rows, ..) => { for r in rows { dfs(r, visited); } }
            }
        }
    }
    dfs(out, &mut visited);
...
impl Adam {
    pub fn new(lr: f64) -> Self { Adam { lr, b1: 0.9, b2: 0.999, eps: 1e-8, m: Default::default(), v: Default::default(), t: 0 } }
    // 更新一组可训练叶(Var 无权父), 用其 .grad
    pub fn step(&mut self, params: &[Var]) {
        self.t += 1;
        let b1 = self.b1; let b2 = self.b2;
    ...
    pub fn load_state(&mut self, params: &[Var], ms: &[Vec<f64>], vs: &[Vec<f64>], t: u64) {
        ...
        for (i, p) in params.iter().enumerate() {
            ...
            // 长度对齐
            let mm = if mm.len() == n { mm } else { let mut z = vec![0.0; n]; z.copy_from_slice(&mm[..mm.len().min(n)]); z };
            let vv = if vv.len() == n { vv } else { let mut z = vec![0.0; n]; z.copy_from_slice(&vv[..vv.len().min(n)]); z };
            nms.insert(id, mm);
            nvs.insert(id, vv);
        }
        self.m = nms;
